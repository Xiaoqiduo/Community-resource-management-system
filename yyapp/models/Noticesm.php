...
class Noticesm extends CI_Model {
	 ...
	 * */
	 public function getNoticesState($notice_state){
	 	$sql ="SELECT a.*,b.notice_type_name, c.state_name, d.street_name, e.cmty_name  
		FROM (((Notices a LEFT JOIN notice_type b ON a.notice_type_id=b.notice_type_id) LEFT JOIN state c ON a.Notice_state=c.state_id) 
		LEFT JOIN community e ON a.cmty_id=e.cmty_id) LEFT JOIN street d ON e.street_id=d.street_id
		WHERE a.notice_state=".$notice_state."";
	 ...
	 public function getNoticesStateType($notice_type_id){
		...
		FROM (((Notices a LEFT JOIN notice_type b ON a.notice_type_id=b.notice_type_id) LEFT JOIN state c ON a.Notice_state=c.state_id)
		 LEFT JOIN community e ON a.cmty_id=e.cmty_id) LEFT JOIN street d ON e.street_id=d.street_id
		 WHERE a.notice_state=1 AND a.notice_type_id=".$notice_type_id."";
	 	$query = $this->db->query($sql);
	 	$row   = $query->result_array();
	 	return $row;
	 }
	...
	public function getNoticesStreetCmtyType($street_id,$cmty_id,$notice_type_id){
		$sql ="SELECT a.*,b.notice_type_name, c.state_name, d.street_name, e.cmty_name, e.street_id  
		FROM (((Notices a LEFT JOIN notice_type b ON a.notice_type_id=b.notice_type_id) LEFT JOIN state c ON a.Notice_state=c.state_id) 
		LEFT JOIN community e ON a.cmty_id=e.cmty_id) LEFT JOIN street d ON e.street_id=d.street_id
		WHERE e.street_id=".$street_id." AND a.cmty_id=".$cmty_id." AND a.notice_type_id=".$notice_type_id."";
		$query = $this->db->query($sql);
		$row   = $query->result_array();
		return $row;
	}
	...
	public function getNoticesId($notice_id){
		$query = $this->db->query("SELECT a.*,b.notice_type_name, c.state_name, d.street_name, e.cmty_name  
		FROM (((Notices a LEFT JOIN notice_type b ON a.notice_type_id=b.notice_type_id) 
		LEFT JOIN state c ON a.Notice_state=c.state_id) LEFT JOIN street d ON a.street_id=d.street_id) 
		LEFT JOIN community e ON a.cmty_id=e.cmty_id WHERE a.notice_id=".$notice_id."");
		$row   = $query->result_array();
		return $row;
	}
